...
def getDefaultUnitName(unitid):
    if IsEUDVariable(unitid):
        raise RuntimeError("EUDVariable is not supported, "\
            "use EUDGetDefaultUnitName_epd instead")
    return defaultUnitNames[unitid]

_db = None

def EUDGetDefaultUnitName_epd(unitid):
    global _db, defaultUnitNames
    if _db is None:
        _db = EUDArray(list(map(EPDConstString, defaultUnitNames)))
    return _db[unitid]